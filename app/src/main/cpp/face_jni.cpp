...
Java_com_example_facerecognitiontest_facenet_FaceNetMobile_FaceModelInit(JNIEnv *env, jobject instance,
...
Java_com_example_facerecognitiontest_facenet_FaceNetMobile_FaceDetect(JNIEnv *env, jobject instance,
...
JNIEXPORT jboolean JNICALL
Java_com_example_facerecognitiontest_facenet_FaceNetMobile_FaceModelUnInit(JNIEnv *env, jobject instance) {
    if (!detection_sdk_init_ok) {
...
Java_com_example_facerecognitiontest_facenet_FaceNetMobile_FaceRecognize(JNIEnv *env, jobject instance,
...
JNIEXPORT jfloatArray JNICALL     //wjy-add
Java_com_example_facerecognitiontest_facenet_FaceNetMobile_FaceEmbFeature(JNIEnv *env, jobject instance,
                                                     jbyteArray faceDate1_, jint w1, jint h1) {
    jbyte *faceDate1 = env->GetByteArrayElements(faceDate1_, NULL);
    unsigned char *faceImageCharDate1 = (unsigned char*)faceDate1;
    ncnn::Mat ncnn_img1 = ncnn::Mat::from_pixels_resize(faceImageCharDate1, ncnn::Mat::PIXEL_RGBA2RGB, w1, h1,112,112);
    ...
    mRecognize->start(ncnn_img1, feature1);
//    for(int i=0;i<128;i++)
//         LOGD("wjy debug, feature1=%f", feature1[i]);
    env->ReleaseByteArrayElements(faceDate1_, faceDate1, 0);
    jfloatArray embfeature = env->NewFloatArray(128);

    float *buffer1 = new float[128];
    if (!feature1.empty())
        memcpy(buffer1, &feature1[0], feature1.size()*sizeof(float));

//    for(int i=0;i<128;i++)
//        LOGD("wjy debug, feature1=%f", buffer1[i]);

    env->SetFloatArrayRegion(embfeature, 0, 128, buffer1);
    return embfeature;
}

JNIEXPORT jdouble JNICALL  //wjy-add
Java_com_example_facerecognitiontest_facenet_FaceNetMobile_calculSimilar(JNIEnv *env, jobject instance,
                                                     jfloatArray v1, jfloatArray v2) {

    jfloat *faceFeaturev1 = env->GetFloatArrayElements(v1, NULL);
    jfloat *faceFeaturev2 = env->GetFloatArrayElements(v2, NULL);

    //if(v1.size() != v2.size()||!v1.size())
    //    return 0;
    double ret = 0.0, mod1 = 0.0, mod2 = 0.0;
    for (std::vector<double>::size_type i = 0; i != 128; ++i)
    {
        ret += faceFeaturev1[i] * faceFeaturev2[i];
        mod1 += faceFeaturev1[i] * faceFeaturev1[i];
        mod2 += faceFeaturev2[i] * faceFeaturev2[i];
    }
